...
c('input[type="number"]');a.length&&(a.each(function(){var a=c(this),b=a.val(),d=a.attr("name"),e=a.attr("id"),h=b+1,f=b-1,g=c("#JsQty").html();g=Handlebars.compile(g);b={key:a.data("id"),itemQty:b,itemAdd:h,itemMinus:f,inputName:d,inputId:e};a.after(g(b)).remove()}),c(".js-qty__adjust").on("click",function(){var a=c(this),b=a.parents(".product-quantity-box").find(".quantity"),d=parseInt(b.val().replace(/\D/g,""));d=r(d);a.hasClass("js-qty__adjust--plus")?d+=1:(--d,1>=d&&(d=1));b.val(d);c(this).closest("form").submit()}))};
